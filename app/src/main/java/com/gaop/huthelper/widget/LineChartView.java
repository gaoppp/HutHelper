...
public class LineChartView extends View {
    ...
    private List<Ranking> rankingList1;
    private List<Ranking> rankingList2;
    ...
    public LineChartView(Context context, @Nullable AttributeSet attrs) {
        ...
        lineWidth = DensityUtils.dp2px(context, 1.5f);
...
//        } else {
    ...
    private void initPaint() {
        ...
        //发光线一号
        BlurMaskFilter bmf = new BlurMaskFilter(5, BlurMaskFilter.Blur.SOLID);
    ...
    protected void onDraw(Canvas ca) {
        if (fullImage == null || shouldUpdate) {
            if (rankingList1 == null || rankingList2 == null || rankingList2.size() == 0 || rankingList1.size() == 0) {
                setVisibility(GONE);
                return;
            } else {
            ...
            //班级排名 计算数据
            int perWidth = (getWidth() - paddingLeft) / rankingList1.size();
            ...
            for (int i = 0; i < rankingList1.size(); i++) {
                ...
                float bjy = startY + ((float) (rankingList1.get(i).getMC() - minNum) / perNum) * linePadding;

                float zyy = startY + ((float) (rankingList2.get(i).getMC() - minNum) / perNum) * linePadding;
                if (i == 0) {
            ...
            for (int i = 0; i < rankingList1.size(); i++) {
                ...
                canvas.drawText(rankingList1.get(i).getMC() + "", cx - 14, bjy - 30, valuePaint1);
                canvas.drawText(rankingList2.get(i).getMC() + "", cx - 14, zyy + 48, valuePaint2);
                if (isXq) {
                ...
                int jdW = rect.width();
                canvas.drawText(rankingList1.get(i).getZJD(), cx - jdW / 2, linePadding * 5 + startY + 2 * ha, textPaint);
                if (i == 0) {
    ...
    public void setRankingData(List<Ranking> list1, List<Ranking> list2, boolean isxq) {
        this.rankingList1 = list1;
        this.rankingList2 = list2;
        this.isXq = isxq;
        shouldUpdate = true;
        initData();
        requestLayout();
